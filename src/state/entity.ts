...
export interface EntityAdapter<T> {
  ...
  selectors: <S>(
    selectEntityState: (state: S) => EntityState<T>
  ) => {
    selectIds: (state: S) => string[];
    selectEntities: (state: S) => { [key: string]: T };
    selectAll: (state: S) => Array<T>;
    selectById: (id: string) => (state: S) => T;
  };
}
...
export function createEntityAdapter<T>(
  ...
  const selectors = <S>(selectEntityState: (state: S) => EntityState<T>) => ({
    selectIds: (state: S) => selectEntityState(state).ids,
    selectEntities: (state: S) => selectEntityState(state).entities,
    selectAll: (state: S) =>
      selectEntityState(state).ids.map(
        id => selectEntityState(state).entities[id]
      ),
    selectById: (id: string) => (state: S) =>
      selectEntityState(state).entities[id]
  });
  return {
    // Initial state
    getInitialState,
